...
!#define DEC
        ...
        Complex (Kind=8), Dimension(:,:), Allocatable :: GRUPB, GRUP, G00UP, G0TUP, &
             & GT0UP, GTTUP, TEMP
        ALLOCATE ( GRUPB(Ndim,Ndim), GRUP(Ndim,Ndim), G00UP(Ndim,Ndim), G0TUP(Ndim,Ndim), &
        ...
        DO NT = NTAUIN, NTAUIN + NTDM -1
           ...
           IF ( MOD(NT,NWRAP).EQ.0) THEN
              ...
              CALL CALCGR(UL, UR, ULRINV, GRUP, TEMP)
           ...
           CALL MMTHR(UR)
            ...
            ENDIF
        ENDDO
        DEALLOCATE (GRUPB, GRUP, G00UP, G0TUP, GT0UP, GTTUP, TEMP )
        RETURN
      END SUBROUTINE DYN
